"""
RAG Engine: ChromaDB-based retrieval for the OpenQA system.
...
class RAGEngine:
    ...
    def retrieve_context(self, query: str, top_k: int = 5, similarity_threshold: float = 0.75) -> list[dict]:
        ...
        Args:
            ...
            similarity_threshold: Distancia coseno máxima permitida.
                Con espacio coseno, distance=0 es coincidencia perfecta y
                distance=2 es completamente opuesto.  Solo se devuelven
                documentos con distance < similarity_threshold.

        Returns:
            Lista de diccionarios con 'text', 'label' y 'distance' por cada resultado.
            Puede estar vacía si ningún documento supera el umbral.
        """
    ...
    def format_rag_prompt(self, query: str, context_list: list[dict]) -> str:
        ...
        Args:
            ...
            context_list: Lista de contextos recuperados (output de retrieve_context).

        Returns:
            Prompt formateado con el contexto inyectado, o el query sin
            contexto si la lista está vacía.
        """
        if not context_list:
