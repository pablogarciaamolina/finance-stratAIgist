...
Takes structured market data from the Market Agent and the user's profile
to produce a personalised investment thesis.
...
class RecommendationAgent:
    ...
    # ------------------------------------------------------------------
    # Market report compression
    # ------------------------------------------------------------------

    def _compress_market_report(self, market_data: Optional[dict]) -> Dict[str, Any]:
        """
        Reduce noise before sending market evidence to Fin-R1.
        Expects the MarketAgent output format:
            {
              "action": ...,
              "result": ...,
              "data": {...market_report...}
